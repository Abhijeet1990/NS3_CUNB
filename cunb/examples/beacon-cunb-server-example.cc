...
NS_LOG_COMPONENT_DEFINE ("CunbServerExample");
...
int main (int argc, char *argv[])
{
  ...
  LogComponentEnable ("CunbServerExample", LOG_LEVEL_ALL);
  ...
  mobilityMs.SetPositionAllocator (positionAllocEnb);
  mobilityMs.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  ...
  OTRHelper oneTimeHelper = OTRHelper ();
  oneTimeHelper.SetSendTime (Seconds (3));
  ApplicationContainer appContainer =oneTimeHelper.Install (endDevices.Get (0));
  ...
  beaconHelper.SetSendTime(Seconds(10));
  ApplicationContainer beaconAppContainer = beaconHelper.Install (enbs.Get (0));
  beaconAppContainer.Start (Seconds (9));
  beaconAppContainer.Stop (Seconds(14));
