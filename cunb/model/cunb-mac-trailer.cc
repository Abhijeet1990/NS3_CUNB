...
NS_OBJECT_ENSURE_REGISTERED (CunbMacTrailer);

const uint16_t CunbMacTrailer::CUNB_MAC_FCS_LENGTH = 2;
const uint16_t CunbMacTrailer::CUNB_MAC_ECC_LENGTH = 2;
const uint16_t CunbMacTrailer::CUNB_MAC_AUTH_LENGTH = 2;

CunbMacTrailer::CunbMacTrailer (void)
  : m_ecc(12),
    m_calcFcs (true)
{
}
...
TypeId
CunbMacTrailer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::CunbMacTrailer")
    .SetParent<Trailer> ()
    .SetGroupName ("cunb")
    .AddConstructor<CunbMacTrailer> ()
  ;
  return tid;
}

TypeId
CunbMacTrailer::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

void
CunbMacTrailer::Print (std::ostream &os) const
{
  os << " FCS = " << m_fcs;
  os << " ECC = " << m_ecc;
  os << " AUTH = " << m_auth;
...
uint32_t
CunbMacTrailer::GetSerializedSize (void) const
{
  return CUNB_MAC_FCS_LENGTH + CUNB_MAC_ECC_LENGTH + CUNB_MAC_AUTH_LENGTH;
}

void
CunbMacTrailer::Serialize (Buffer::Iterator start) const
{
...
uint32_t
CunbMacTrailer::Deserialize (Buffer::Iterator start)
{
  start.Prev (CUNB_MAC_FCS_LENGTH + CUNB_MAC_ECC_LENGTH + CUNB_MAC_AUTH_LENGTH);
...
uint16_t
CunbMacTrailer::GetFcs (void) const
{
  return m_fcs;
}

void
CunbMacTrailer::SetFcs (Ptr<const Packet> p)
{
  if (m_calcFcs)
    {
...
bool
CunbMacTrailer::CheckFcs (Ptr<const Packet> p)
{
  if (!m_calcFcs)
    {
      return true;
...
void
CunbMacTrailer::EnableFcs (bool enable)
{
  m_calcFcs = enable;
  if (!enable)
    {
...
uint16_t
CunbMacTrailer::GetAuth(void) const
{
	return m_auth;
}

void
CunbMacTrailer::SetAuth (Ptr<const Packet> p)
{
...
bool CunbMacTrailer::CheckAuth (Ptr<const Packet> p)
{
	uint16_t checkAuth;
	uint16_t size = p->GetSize ();
	uint8_t *serial_packet = new uint8_t[size];
...
bool
CunbMacTrailer::IsFcsEnabled (void)
{
  return m_calcFcs;
}

uint8_t
CunbMacTrailer::GenerateCrc8 (uint8_t *data, int length)
{
...
uint16_t
CunbMacTrailer::GenerateCrc16 (uint8_t *data, int length)
{
  int i;
...
uint16_t
CunbMacTrailer::GenerateHash(uint8_t* data, int length)
{
