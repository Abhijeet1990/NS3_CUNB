...
NS_LOG_COMPONENT_DEFINE ("CunbChannel");

NS_OBJECT_ENSURE_REGISTERED (CunbChannel);

TypeId
CunbChannel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::CunbChannel")
    .SetParent<Channel> ()
    .SetGroupName ("cunb")
    .AddConstructor<CunbChannel> ()
    .AddAttribute ("PropagationLossModel",
                   "A pointer to the propagation loss model attached to this channel.",
                   PointerValue (),
                   MakePointerAccessor (&CunbChannel::m_loss),
                   MakePointerChecker<PropagationLossModel> ())
    .AddAttribute ("PropagationDelayModel",
                   "A pointer to the propagation delay model attached to this channel.",
                   PointerValue (),
                   MakePointerAccessor (&CunbChannel::m_delay),
                   MakePointerChecker<PropagationDelayModel> ())
    .AddTraceSource ("PacketSent",
                     "Trace source fired whenever a packet goes out on the channel",
                     MakeTraceSourceAccessor (&CunbChannel::m_packetSent),
                     "ns3::Packet::TracedCallback");
  return tid;
}

CunbChannel::CunbChannel ()
{
}
...
uint32_t
CunbChannel::GetNDevices (void) const
{
  return m_phyList.size ();
}

Ptr<NetDevice>
CunbChannel::GetDevice (uint32_t i) const
{
  return m_phyList[i]->GetDevice ()->GetObject<NetDevice> ();
}

void
CunbChannel::Send (Ptr< CunbPhy > sender, Ptr< Packet > packet,
      ...
      if (sender != (*i))
        {
          ...
          CunbChannelParameters parameters;
...
void
CunbChannel::Receive (uint32_t i, Ptr<Packet> packet,
                      CunbChannelParameters parameters) const
{
  NS_LOG_FUNCTION (this << i << packet << parameters);

  // Call the appropriate PHY instance to let it begin reception
  m_phyList[i]->StartReceive (packet, parameters.rxPowerDbm,
                              parameters.duration, parameters.frequencyMHz);
}

double
CunbChannel::GetRxPower (double txPowerDbm, Ptr<MobilityModel> senderMobility,
