...
{
  // these are set for DL-Physical Layer
  double bitrate = 600.0;
  uint32_t nPreamble = 4; //!< Number of preamble symbols
  bool eccEnabled = 1; //!< Whether Cyclic Redundancy Check is enabled
  bool authEnabled = 1;
  bool fcsEnabled = 1;
  uint32_t nSeqCounter = 6; // Number of symbols for Sequence Counter, used only for Uplink transmission
  uint32_t nMsId = 20; // Number of symbols required for MS ID.
};
...
/**
 * Allow logging of CunbTxParameters like with any other data type.
 */
 ...
 * \ingroup cunb
 *
 * Base class for PHY layers implementing the Cunb modulation scheme.
 *
...
public:
   ...
   *
   * This method is typically called by CunbChannel.
   *
   ...
   * passing a CunbInterferenceHelper Event to this method, the class will be
  ...
  virtual void EndReceive (Ptr<Packet> packet,
                           Ptr<CunbInterferenceHelper::Event> event) = 0;
  ...
  /**
     * Whether this device is receiving or not.
     *
     * \returns true if the device is currently receiving a packet, false
     * otherwise.
     */
    //virtual bool IsReceiving (void) = 0;
  ...
  /**
   * Set the CunbChannel instance PHY transmits on.
   *
   * Typically, there is only one instance per simulation.
   *
   * \param channel The CunbChannel instance this PHY will transmit on.
   */
   ...
   *
   * \return The CunbChannel instance this PHY transmits on.
   */
  Ptr<CunbChannel> GetChannel (void) const;
   ...
   * transmitted.
   *
   * Besides from the ones saved in CunbTxParameters, the packet's payload
   ...
   */
  static Time GetOnAirTime (Ptr<Packet> packet, CunbTxParameters txParams, enum DeviceType dType);
...
protected:
  ...
  Ptr<CunbChannel> m_channel; //!< The channel this PHY transmits on.

  CunbInterferenceHelper m_interference; //!< The CunbInterferenceHelper
