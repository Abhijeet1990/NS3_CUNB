#ifndef CUNB_DEVICE_ADDRESS_H
#define CUNB_DEVICE_ADDRESS_H
...
public:
   ...
   */
  CunbDeviceAddress (uint32_t address);

  /**
   * Build a new address from a network id and network address.
   */
  CunbDeviceAddress (uint8_t nwkId, uint32_t nwkAddr);

  /**
   * Build a new address from a network id and network address.
   */
  CunbDeviceAddress (NwkID nwkId, NwkAddr nwkAddr);
   ...
   */
  static CunbDeviceAddress Deserialize (const uint8_t buf[4]);

  /**
   * Convert from an ordinary address to a LoraDeviceAddress instance.
   */
  static CunbDeviceAddress ConvertFrom (const Address &address);
  ...
  bool operator== (const CunbDeviceAddress &other) const;
  bool operator!= (const CunbDeviceAddress &other) const;
  bool operator< (const CunbDeviceAddress &other) const;
  bool operator> (const CunbDeviceAddress &other) const;
...
private:
  /**
   * Convert this instance of CunbDeviceAddress to an Address
   */
