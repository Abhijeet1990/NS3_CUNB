...
NS_LOG_COMPONENT_DEFINE ("CunbMacCommand");

NS_OBJECT_ENSURE_REGISTERED (CunbMacCommand);

TypeId
CunbMacCommand::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::CunbMacCommand")
    .SetParent<Object> ()
    .SetGroupName ("cunb")
  ;
  return tid;
...
enum MacCommandType
CunbMacCommand::GetCommandType (void) const
{
...
uint8_t
CunbMacCommand::GetSerializedSize (void) const
{
  NS_LOG_FUNCTION_NOARGS ();
...
uint8_t
CunbMacCommand::GetCIDFromMacCommand (enum MacCommandType commandType)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
LinkCheckAns::SetEnbCnt (uint8_t gwCnt)
{
...
uint8_t
LinkCheckAns::GetEnbCnt (void) const
{
  NS_LOG_FUNCTION (this);
...
void
LinkCheckAns::IncrementEnbCnt (void)
{
  NS_LOG_FUNCTION (this);
...
RxParamSetupReq::RxParamSetupReq (uint8_t rx2DataRate,
                                  double frequency) :
  m_rx2DataRate (rx2DataRate),
  m_frequency (frequency)
{
  NS_LOG_FUNCTION (this << unsigned (rx2DataRate) <<
                   frequency);
...
RxParamSetupReq::Serialize (Buffer::Iterator &start) const
{
  ...
  start.WriteU8 (GetCIDFromMacCommand (m_commandType));
  // Data serialization
  start.WriteU8 (m_rx2DataRate & 0b1111);
...
RxParamSetupAns::RxParamSetupAns (bool rx2DataRateAck,
                                  bool channelAck) :
  m_rx2DataRateAck (rx2DataRateAck),
  m_channelAck (channelAck)
{
  NS_LOG_FUNCTION (this << rx2DataRateAck << channelAck);
...
RxParamSetupAns::Serialize (Buffer::Iterator &start) const
{
  ...
  start.WriteU8 (GetCIDFromMacCommand (m_commandType));
  // Data serialization
  start.WriteU8 (uint8_t (m_rx2DataRateAck) << 1 |
